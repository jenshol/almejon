...
- Avomeri käyttää Open-Meteon oletusarvoista `best_match`-mallivalintaa (ei pakotettua `models=`-parametria) — sille tämä on jo osoittautunut identtiseksi `meteofrance_wave`-mallin kanssa (~8 km, täysi 7 vrk:n horisontti). Rannikko sen sijaan **yhdistää kaksi mallia** (`mergeMarineForecasts()`): `dwd_ewam` (DWD:n EWAM, n. 5 km hila) jokaiselle tunnille jolla sillä on dataa, ja `best_match` täydentämään loput 7 vrk:n pyynnöstä. Syy: kokeiltiin ensin pelkkää `dwd_ewam`:a, mutta sen ennustehorisontti osoittautui tälle pisteelle vain n. 3,4 vuorokaudeksi (loput tulivat null-arvoina, jolloin "Seuraavat vuorokaudet" näytti dataa vain lähipäiviin). Käytännön vertailussa `dwd_ewam` kuitenkin osui lähitunneille selvästi paremmin yhteen sekä Aaltopoiju-havaintohistorian että käyttäjän omien havaintojen kanssa kuin `best_match` (joka tälle pisteelle = `meteofrance_wave`) — joten yhdistelmä käyttää tarkempaa mallia niin pitkälle kuin sillä on dataa, ja karkeampaa (mutta silti käyttökelpoista) mallia sen jälkeen kauemmas tulevaisuuteen, missä ennuste on joka tapauksessa jo epävarma ja muuttuu usein. `loadMarineForecast(lat, lon, model)` ottaa edelleen valinnaisen kolmannen parametrin. Muita Open-Meteon tarjoamia merimalleja: `dwd_gwam` (~25 km), `ecmwf_wam` (9 km, koko maapallo), `era5_ocean` (historiadata, ~50 km).
