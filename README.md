...
- Rannikon merienuste (`loadMarineForecast`) pyytää nimenomaan `models=dwd_ewam` (DWD:n EWAM, n. 5 km hila — tarkin Euroopan/Itämeren alueella tarjolla oleva Open-Meteon aaltomalli) oletusarvoisen `best_match`-automaattivalinnan sijaan. Avomerellä ei ole pakotettu mallia, koska `best_match` on siellä jo osoittautunut hyväksi eikä sitä kannattanut korjata. Muita Open-Meteon tarjoamia merimalleja: `meteofrance_wave` (~8 km), `dwd_gwam` (~25 km), `ecmwf_wam` (9 km, koko maapallo), `gfs_wave_016`/`gfs_wave_025` (NCEP), `era5_ocean` (historiadata, ~50 km) — näistä `dwd_ewam` on ainoa jonka dokumentaatio nimeää tarkimmaksi juuri Euroopan rannikoilla.
