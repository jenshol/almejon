...
- Molemmat sijainnit käyttävät Open-Meteon oletusarvoista `best_match`-mallivalintaa (ei pakotettua `models=`-parametria). Kokeilimme aluksi pakottaa rannikolle `dwd_ewam`:n (DWD:n EWAM, n. 5 km hila — dokumentaation mukaan tarkin Euroopan/Itämeren alueella tarjolla oleva Open-Meteon aaltomalli), mutta tälle pisteelle sen ennustehorisontti osoittautui vain n. 3,4 vuorokaudeksi (loput 7 vrk:n pyynnöstä tulivat null-arvoina — siksi "Seuraavat vuorokaudet" näytti dataa vain sunnuntaihin asti). Vertailtaessa `best_match` osoittautui tälle pisteelle täsmälleen samaksi kuin `meteofrance_wave` (0 eroa 168 tunnin yli koko 7 vrk:n ajalta) — eli valmiiksi kohtuu hienojakoinen (~8 km) JA täyden ennustepituuden antava, ilman erillistä pakotusta. Siksi malliparametri poistettiin kokonaan käytöstä. `loadMarineForecast(lat, lon, model)` tukee edelleen valinnaista kolmatta parametria, jos jotain mallia joskus halutaan kokeilla uudelleen. Muita Open-Meteon tarjoamia merimalleja: `dwd_ewam` (~5 km, mutta lyhyt horisontti tässä pisteessä), `dwd_gwam` (~25 km), `ecmwf_wam` (9 km, koko maapallo), `era5_ocean` (historiadata, ~50 km).
