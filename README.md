...
- `classify(heightM, periodS, windMs)` — palauttaa jatkuvan mukavuusindeksin `{level, score, ...}` (ei portaittaisia pykälänostoja). Kolme osatekijää, kukin oma kaava suoraan ilman turvamarginaaleja, lopputulos on osatekijöiden huonoin (max):
  ...
  Pallon väri on tasaisen kirkas vihreä indeksiin 1,7 asti ja tasaisen keltainen indeksistä 1,9 alkaen — vain kapealla välillä 1,7–1,9 (`GRADIENT_START`/`GRADIENT_END` `greenYellowBlend()`-funktiossa) väri liukuu portaattomasti vihreästä keltaiseen, jotta juuri "onko tämä vielä ihan hyvä vai jo selvästi välttävä" -raja ei tunnu mielivaltaisen terävältä. Välttävä→Huono-raja (2,0) on edelleen täysin terävä, ei liukua.
